...
* **Development database port collision (discovered during I-2):** this development machine runs two independent Postgres 16 servers both listening on port 5432 — a Docker container (reachable only via `docker exec`) and a real Postgres instance inside a WSL Ubuntu distro (reachable via `localhost:5432` from every host-side tool, since Windows prefers the more specific `127.0.0.1` binding over Docker's `0.0.0.0` one). The Docker container proved to be a stale, unused artifact never touched since initial `docker-compose` setup; the WSL instance is the real database behind all local development and the already-submitted Lab 2 report. `docker-compose.yml`'s `db` service now publishes host port 5433 instead of 5432 to remove the ambiguity; its internal container-to-container traffic is unaffected. The Phase 0 backup and regression baseline in `tests.md` were retaken against the correct (WSL) database after this was found.
* **Idempotent seed data:** `prisma/seed.ts` was changed from a destructive delete-then-recreate pattern (which would have wiped any ticket created through real app usage on every seed run) to an additive upsert/skip-if-exists pattern, per §5.3's "safe to run repeatedly" requirement. Verified idempotent by running it twice consecutively with identical resulting counts.
* **API response compatibility during migration (I-2):** the Prisma field rename (`priority` → `requestedPriority`) and the `status`/`priority` column type change (`String` → native enum) are internal to the data layer. The Lab 2 REST API's JSON response shape (`priority`, `status` as Title-Case/underscore strings) is preserved unchanged via a `serializeTicket` mapping in `app.ts`, so the existing Lab 2 client and its 18 automated tests continue to pass without modification. This is a deliberate, temporary compatibility shim; I-4 (Requester regression) is the issue that reworks the API contract and client together.
