...
| UI-03e | UI Component | FR-14 (added in review of PR #67 — item 1: "Open" navigated to a route App.tsx never defines, and no test caught it; **superseded in I-7**, which built that route) | Queue "Open" action | Clicking a row's Open button navigates to the real `/staff/tickets/:id` Staff Ticket Detail screen (I-7); the read-only modal from I-6 remains available only via the optional `onOpenTicket` override, no longer the default | `client/tests/lab-03/StaffTicketQueue.test.tsx` (item-1) | Pass |
